...
export const CometCard = ({
  ...
  const [showSkillsUnderline, setShowSkillsUnderline] = useState(false)
        ...
        const handleScroll = () => {
            ...
            if (skillsContent) {
                if (shouldExpand) {
                    ...
                    setShowSkillsUnderline(true)
                    
                    // Trigger underline animation by dispatching custom event
                    setTimeout(() => {
                        window.dispatchEvent(new CustomEvent('triggerSkillsUnderline'))
                    }, 500)
                    ...
                    setTimeout(() => {
                        ...
                        skillItems.forEach((item, index) => {
                            ...
                            htmlItem.style.transition = `all 0.8s ease ${index * 0.15}s`
                        })

                        skillProgress.forEach((progress, index) => {
                            const htmlProgress = progress as HTMLElement
                            const level = progress.getAttribute('data-level') || htmlProgress.style.width
                            htmlProgress.style.width = level
                            htmlProgress.style.transition = `width 1.5s ease ${0.5 + index * 0.1}s`
                        })
                    }, 100)
                } else {
                    skillsContent.classList.remove('opacity-100')
                    skillsContent.classList.add('opacity-0')
                    setShowSkillsUnderline(false)
                }
            }
        }
