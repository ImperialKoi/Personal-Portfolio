...
const BOARD_WIDTH = 40 // Increased from 38 to 40 to fix right border
...
export const useSnakeGame = () => {
  ...
  const renderGame = useCallback(() => {
    ...
    const lines = [
      "🐍 SNAKE GAME",
      "━".repeat(BOARD_WIDTH + 4), // Fixed border length
      "┌" + "─".repeat(BOARD_WIDTH) + "┐", // Fixed top border
      ...board.map((row) => `│${row.join("")}│`),
      "└" + "─".repeat(BOARD_WIDTH) + "┘", // Fixed bottom border
      "",
  ...
  // Game loop - Fixed timing issue
  useEffect(() => {
    if (!gameState.isPlaying || gameState.gameOver) return

    const interval = setInterval(moveSnake, 150) // Consistent timing for all directions
